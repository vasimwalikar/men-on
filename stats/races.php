            ...
            <div class="templatemo-content-widget white-bg col-2">
             ...
             <!-- <div class="square"></div>-->
			  ...
			  <?php
					...
					///////////dropdown value for Race Type //////////////
					$winner = $_POST[winner] ;
					
					///////////dropdown value for Number of Player//////////////
					$numplayers = $_POST[player] ;
					...
					if ($username != null) {
						$racelogs = mysql_query("SELECT * FROM racelog WHERE start BETWEEN '$from_date' AND '$to_date' (player1='".$username."' OR player2='".$username."' OR player3='".$username."' OR player4='".$username."')order by raceid desc ".(isset($platform) ? " AND type = '".$platform."'" : ""));
						...
						$racelogs = mysql_query("SELECT * FROM racelog WHERE numplayers = '$numplayers' AND start BETWEEN '$from_date' and '$to_date' order by raceid desc".(isset($platform) ? " WHERE type = '".$platform."'" : ""));
						//echo "<h3>Complete list of ".(isset($platform) ? $platform." " : "")."races</h3>";
						
						if($numplayers == 0){
							$racelogs = mysql_query("SELECT * FROM racelog WHERE numplayers != '' AND start BETWEEN '$from_date' and '$to_date' order by raceid desc".(isset($platform) ? " WHERE type = '".$platform."'" : ""));
						}
					
						if($winner == 1){
							$racelogs = mysql_query("SELECT * FROM racelog WHERE winner IS NOT NULL AND numplayers = '$numplayers' AND start BETWEEN '$from_date' and '$to_date' order by raceid desc".(isset($platform) ? " WHERE type = '".$platform."'" : ""));
							//echo "<h3>Complete list of ".(isset($platform) ? $platform." " : "")."races</h3>";
						}
						
						if($winner == 2){
							$racelogs = mysql_query("SELECT * FROM racelog WHERE winner IS NULL AND numplayers= '$numplayers' AND start BETWEEN '$from_date' and '$to_date' order by raceid desc".(isset($platform) ? " WHERE type = '".$platform."'" : ""));
							//echo "<h3>Complete list of ".(isset($platform) ? $platform." " : "")."races</h3>";
						}
						
						if($winner == 1 && $numplayers == 0){
							$racelogs = mysql_query("SELECT * FROM racelog WHERE winner IS NOT NULL AND numplayers != '' AND start BETWEEN '$from_date' and '$to_date' order by raceid desc".(isset($platform) ? " WHERE type = '".$platform."'" : ""));
						}
						
						if($winner == 2 && $numplayers == 0){
							$racelogs = mysql_query("SELECT * FROM racelog WHERE winner IS NULL AND numplayers != '' AND start BETWEEN '$from_date' and '$to_date' order by raceid desc".(isset($platform) ? " WHERE type = '".$platform."'" : ""));
						}
					}
					...
					<div class="col-lg-12 form-group" style="padding-left: 25px; padding-right: 25px; margin-top: 200px;">  
						<div class="col-lg-6 col-md-6 form-group" style="width: 200px;"> 
							...
							<select class="form-control" name="winner">
								<option VALUE="" <?php echo $Wave == '0' ? 'selected' : '';  ?>>All</option>
						...
						<div class="col-lg-6 col-md-6 form-group" style="width: 200px;"> 
							...
							<select class="form-control" name="player">
								...
								<option VALUE ="4" <?php echo $Wave_player== '4' ? 'selected': ''; ?>>4-Player</option>              
							</select>
						</div>
